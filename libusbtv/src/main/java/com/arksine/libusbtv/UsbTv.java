...
public class UsbTv extends Handler {
    ...
    public static boolean open(@NonNull final Context appContext, @NonNull DeviceParams params) {
        synchronized (OPEN_LOCK) {
            ...
            if (dev == null) {
                ...
                ((Activity) appContext).runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        Toast.makeText(appContext, "Error, UsbDevice not set in DeviceParams.  Cannot open", Toast.LENGTH_SHORT);
                    }
                });
...
//            }
     ...
     */
    public static ArrayList<UsbDevice> enumerateUsbtvDevices(final Context context) {
        ArrayList<UsbDevice> devList = new ArrayList<>(3);

        UsbManager manager = (UsbManager) context.getSystemService(Context.USB_SERVICE);
        final HashMap<String, UsbDevice> usbMap = manager.getDeviceList();
        ...
        ((Activity) context).runOnUiThread(new Runnable() {
            @Override
            public void run() {
                Toast.makeText(context,"Checking for Usb Devices, Number reported: " + usbMap.size(), Toast.LENGTH_SHORT).show();
            }
        });

        for (final UsbDevice dev : usbMap.values()) {
            ...
            ((Activity) context).runOnUiThread(new Runnable() {
                ...
                @Override
                public void run() {
                    Toast.makeText(context, dev.getDeviceName() + ";;; Vendor Id: " + dev.getVendorId() + "ProductId;;" + dev.getProductId() + usbMap.size(), Toast.LENGTH_SHORT).show();
                }
            });
            ...
            if (dev.getVendorId() == 0x1b71  && dev.getProductId() == 0x3002) {
                ...
                ((Activity) context).runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        Toast.makeText(context, "0x1b71 added", Toast.LENGTH_SHORT).show();
                    }
                });
            }
        }
    ...
    public static UsbDevice enumerateUsbtvDevice(final Context context) {
        ArrayList<UsbDevice> devList = new ArrayList<>();
        ...
        for (final UsbDevice dev : usbMap.values()) {
            ...
            if (dev.getVendorId() == 7304 /* && dev.getProductId() == 74*/) {
                usbDevice = dev;
                ((Activity) context).runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        Toast.makeText(context, "7304 added", Toast.LENGTH_SHORT).show();
                    }
                });
            }
        }
